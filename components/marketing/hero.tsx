...
export function Hero() {
  return (
    <section className="relative flex min-h-[calc(100dvh-65px)] items-center overflow-hidden bg-gradient-to-b from-primary-50 via-cream-50 to-cream-50">
      ...
      <div className="mx-auto w-full max-w-7xl px-4 py-4 sm:px-6 sm:py-8 lg:px-8 lg:py-12">
        <div className="grid items-center gap-6 lg:grid-cols-2 lg:gap-16">
          <div className="flex flex-col items-center text-center lg:items-start lg:text-left">
            <span className="inline-flex items-center gap-2 rounded-full border border-primary-200 bg-cream-50 px-4 py-1.5 text-xs font-medium text-primary-800 shadow-sm sm:text-sm">
              <Users className="h-4 w-4 text-primary-600" />
              Pensée pour les musulmans d&apos;Afrique
            </span>

            <h1 className="mt-3 text-2xl font-bold tracking-tight text-primary-900 sm:mt-4 sm:text-3xl lg:mt-6 lg:text-5xl">
              Et si la personne qu&apos;<span className="text-primary-600">Allah</span> a mise sur
              votre chemin vous attendait ici&nbsp;?
            </h1>

            <p className="mt-2 max-w-2xl text-xs leading-relaxed text-primary-900/70 sm:mt-3 sm:text-base lg:mt-6 lg:text-lg">
            ...
            <div className="mt-4 flex flex-col items-center gap-4 sm:flex-row lg:mt-9">
              <Link
                href="/signup"
                className="w-full rounded-full bg-primary-600 px-8 py-2.5 text-center text-base font-semibold text-cream-50 shadow-lg shadow-primary-600/20 transition hover:bg-primary-700 sm:w-auto"
              >
                Commencer sur le web
              </Link>
            </div>

            <div className="mt-4 flex flex-wrap items-center justify-center gap-x-6 gap-y-2 text-xs font-medium text-primary-900/70 sm:mt-6 sm:gap-x-8 sm:text-sm lg:mt-10 lg:justify-start">
          ...
          <div className="relative mx-auto w-full max-w-[220px] sm:max-w-xs lg:max-w-sm">
            ...
            <div className="absolute -bottom-4 left-1/2 flex w-[85%] -translate-x-1/2 items-center gap-3 rounded-2xl border border-primary-100 bg-cream-50/95 px-4 py-2.5 shadow-xl backdrop-blur sm:-bottom-5 sm:px-5 sm:py-3.5">
              <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-primary-100 text-primary-600 sm:h-9 sm:w-9">
                <ShieldCheck className="h-3.5 w-3.5 sm:h-4.5 sm:w-4.5" />
              </span>
              <p className="text-[10px] font-medium text-primary-900 sm:text-xs">
